...
{*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------}
   ...
   public
      ...
      procedure AddAppender(AAppender : TAppender);
      procedure RemoveAppender(const AName : String);
      procedure RemoveAllAppenders();
      function GetAppender(const AName : String) : TAppender;
      function GetAllAppenders() : TStrings;
...
{*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------}
procedure TLogger.AddAppender(AAppender : TAppender);
begin
   FAppenders.AddObject(AAppender.getName, AAppender);
...
var
   index : Integer;
begin
   index := FAppenders.IndexOf(AName);
   if (index >= 0) then begin
      FAppenders.Objects[index].Free;
      FAppenders.Delete(index);
   end;
...
{*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------}
function TLogger.GetAppender(const AName : String) : TAppender;
var
   index : Integer;
begin
   index := FAppenders.IndexOf(AName);
   if (index >= 0) then
      getAppender := TAppender(FAppenders.Objects[index])
   else
      getAppender := Nil;
end;
...
{*----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------}
function TLogger.GetAllAppenders() : TStrings;
begin
...
begin
   if (AEvent.getLevel.isGreaterOrEqual(Self.FLevel)) then
      for i := 0 to FAppenders.Count-1 do
         TAppender(FAppenders.Objects[i]).doAppend(AEvent);
end;
