...
var
    I: integer;
begin
    GetEtherProtoName := IntToHex (protocol, 4) ;
    for I := 1 To SizeOf (EtherProto) div SizeOf (TEtherProto) do
    begin
        if protocol = EtherProto [I].itype then GetEtherProtoName := EtherProto [I].iName ;
    end ;
end;
...
var
    I: integer;
begin
    GetIPProtoName := IntToStr (protocol) ;
    for I := 1 To SizeOf (IPPROTO) div SizeOf (TIPProto) do
    begin
        if protocol = IPPROTO [I].itype then GetIPProtoName := IPPROTO [I].iName ;
    end ;
end;
...
begin
    ...
    if (result = '') and (s_port < 1024) then GetServiceName := '<' + IntToStr (s_port) + '>' ;
    if (result = '') and (d_port < 1024) then GetServiceName := '<' + IntToStr (d_port) + '>' ;
end ;
...
begin
    GetICMPType := 'UNKNOWN';
    case x of
     0: GetICMPType := 'ECHO_REPLY'; // Echo Reply
     3: GetICMPType := 'DEST-UNREA'; // Destination Unreachable
     4: GetICMPType := 'SRC_Q';  // Source Quench
     5: GetICMPType := 'REDIR';  // Redirect
     8: GetICMPType := 'ECHO';   // Echo
    11: GetICMPType := 'TTLX';   // Time Exceeded
    12: GetICMPType := 'BADPAR'; // Parameter Problem
    13: GetICMPType := 'TIME';   // Timestamp
    14: GetICMPType := 'TIME_REPLY'; // Timestamp Reply
    15: GetICMPType := 'INFO';   // Information Request
    16: GetICMPType := 'INFO_REPLY'; // Information Reply
   end ;
end ;
